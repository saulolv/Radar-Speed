...
ZTEST(radar_unit, test_vehicle_classification)
{
    ...
    enum vehicle_type type_light = (axles_light <= 2) ? VEHICLE_LIGHT : VEHICLE_HEAVY;
    ...
    enum vehicle_type type_moto = (axles_moto <= 2) ? VEHICLE_LIGHT : VEHICLE_HEAVY;
    zassert_equal(type_moto, VEHICLE_LIGHT, "1 axle should be LIGHT");
    
    uint32_t axles_heavy = 3;
    enum vehicle_type type_heavy = (axles_heavy <= 2) ? VEHICLE_LIGHT : VEHICLE_HEAVY;
    zassert_equal(type_heavy, VEHICLE_HEAVY, "3 axles should be HEAVY");
    
    uint32_t axles_truck = 5;
    enum vehicle_type type_truck = (axles_truck <= 2) ? VEHICLE_LIGHT : VEHICLE_HEAVY;
